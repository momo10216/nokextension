...
class NoKExtensionModelExtensions extends JModelList {
	private $tableName = '#__nok_extensions';
	...
	protected function getListQuery() {
		...
		$query
			->select($db->quoteName(array('e.id', 'e.name')))
		...
		if (!empty($search)) {
			...
			if (stripos($search, 'id:') === 0) {
				$query->where('e.id = ' . (int) substr($search, 3).$whereExt);
			} else {
				$search = $db->quote('%' . $db->escape($search, true) . '%');
				$query->where('(e.name LIKE ' . $search . ')'.$whereExt);
			}
		} else {
			if (!empty($whereExt)) {
		...
		$orderColText = $this->state->get('list.ordering', 'e.name');
	...
	public function getExportData($parentId='') {
		...
		foreach($rows as $key => $row) {
			if (!empty($row['asset_id'])) {
				$assetRow = $this->_getAssetRowByName('com_nokextension.extension.'.$row['id']);
				$json = $assetRow['rules'];
	...
	public function importPostSave($row, $id) {
		if (empty($this->_componentAssetId)) {
			$assetRow = $this->_getAssetRowByName('com_nokextension');
			$this->_componentAssetId = $assetRow['id'];
		}
		$this->_setAssetRulesById($row['asset_id'],$this->_componentAssetId,'com_nokextension.extension.'.$id, $row['name'], $this->_mapGroupTitle2Id($this->_assetRules));
	}
