...
class NoKExtensionModelExtension extends JModelForm {
	/**
	 * @since   1.6
	 */
	private $pk = '0';
	private $useAlias= true;
	protected $view_item = 'extension';
	protected $_item = null;
	protected $_membershipItems = null;
	protected $_model = 'container';
	protected $_component = 'com_nokextension';
	protected $_context = 'com_nokextension.extension';
	protected $_taskItems = null;

	private function getFields() {
		$params = JComponentHelper::getParams($this->_component);
		return array (
			'id' => array(JText::_('COM_NOKEXTENSION_COMMON_FIELD_ID_LABEL',true),'`e`.`id`'),
			'name' => array(JText::_('COM_NOKEXTENSION_CONTAINER_FIELD_NAME_LABEL',true),'`e`.`name`'),
			'createdby' => array(JText::_('COM_NOKEXTENSION_COMMON_FIELD_CREATEDBY_LABEL',true),'`e`.`createdby`'),
			'createddate' => array(JText::_('COM_NOKEXTENSION_COMMON_FIELD_CREATEDDATE_LABEL',true),'`e`.`createddate`'),
			'modifiedby' => array(JText::_('COM_NOKEXTENSION_COMMON_FIELD_MODIFIEDBY_LABEL',true),'`e`.`modifiedby`'),
			'modifieddate' => array(JText::_('COM_NOKEXTENSION_COMMON_FIELD_MODIFIEDDATE_LABEL',true),'`e`.`modifieddate`')
		);
	}
	 ...
	 */
	public function getTable($type = 'Extensions', $prefix = 'NoKExtensionTable', $config = array()) {
	...
	public function &getItem($pk = null) {
		...
		if (!isset($this->_item[$pk])) {
			try {
				...
				$query->select($fields)
					->from($db->quoteName('#__nok_extensions','e'))
					->where('e.id = ' . (int) $pk);
				$db->setQuery($query);
				$data = $db->loadObject();
				$this->_item[$pk] = $data;
	...
	public function getItemByName($name) {
		try {
			...
			$query->select($fields)
				->from($db->quoteName('#__nok_extensions','e'))
				->where($db->quoteName('e.name').' = '.$db->quote($name));
			$db->setQuery($query);
			$data = $db->loadObject();
	...
	public function translateFieldsToColumns($fields, $removePrefix=true) {
		...
		foreach($fields as $field) {
			if (isset($allFields[$field]) && !empty($allFields[$field])) {
				if ($removePrefix) {
					$resultField = str_replace('`e`.', '' , $allFields[$field][1]);
